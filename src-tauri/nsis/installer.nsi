...
; ToolDock intentionally defaults x64 per-machine installs to Program Files (x86).
Unicode true
...
Section Install
  ...
  ; Stop the privileged sensor task before replacing its executable.
  ...
  ; LibreHardwareMonitor 0.9.6 reads privileged sensors through the signed
  ; PawnIO driver. Install the standard edition; never enable unrestricted mode.
  nsExec::ExecToLog '"$INSTDIR\binaries\PawnIO_setup.exe" -install -silent'
  Pop $0
  ${If} $0 != 0
  ${AndIf} $0 != 3010
    DetailPrint "PawnIO installation returned exit code $0. CPU temperature and fan data may be unavailable."
  ${EndIf}
  ...
  FileWriteUTF16LE $1 '  <Actions Context="Author"><Exec><Command>$INSTDIR\tooldock-hardware-monitor.exe</Command><Arguments>--output-file "$APPDATA\ToolDock\hardware-sensors.json" --interval-ms 2000</Arguments></Exec></Actions>$\r$\n'
